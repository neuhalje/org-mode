...
- =EXPORT_FILE_NAME= ::
  ...
  The filename should not have an extension. E.g. an
  =EXPORT_FILE_NAME= of ~my document~ would export to
  ~my document.pdf~ (or whatever the exporter produces).

  If the exported filename should contain dots, e.g.
  ~my document v.2~, then a (rather hackish) trick is to
  end the filename with a dot:

  #+begin_example
  ,#+EXPORT_FILE_NAME: my document v.2.
  #+end_example

  If =EXPORT_FILE_NAME= is enclosed in backticks then the text
  between the ticks is evaluated end the returned value
  is used as filename.

  A common use case when writing documentation is that different
  versions of the document will be exported over time. These are
  then distributed, e.g. via e-mail or archived.

  By deriving descriptive names during export, the exchange of
  the document is greatly simplified.

  E.g. the following excerpt will create a filename that contains
  the date of export, the document title and the document version:
  ...
  The /trailing dot hack/ is very helpful when generating the
  filename. In those cases the dot needs to be /after/ the closing
  backtick:

  #+begin_example
  ,#+EXPORT_FILE_NAME: `(format "%s" "my document v.2")`.
  #+end_example
